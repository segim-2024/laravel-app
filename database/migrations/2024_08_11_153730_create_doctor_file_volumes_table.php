...
return new class extends Migration
{
    ...
    {
        Schema::create('doctor_file_volumes', function (Blueprint $table) {
            $table->id();
            $table->uuid('volume_uuid')->unique()->comment('자료박사 Volume UUID');
    ...
    {
        Schema::table('doctor_file_volumes', function (Blueprint $table) {
            // 외래키 관계를 선언했다면, 리버스 마이그레이션 할때 에러를 피하기 위해
            // 테이블을 삭제하기 전에 외래키를 먼저 삭제하는 것이 중요하다.
            $table->dropForeign('doctor_file_volumes_series_uuid_foreign');
        });

        Schema::dropIfExists('doctor_file_volumes');
    }
};
